...
export const showProfileData = async (email) => {
  ...
  const defaultAddres = await addressModel.findOne({
    userId: userId._id,
    isDefault: true,
  });
...
export const allAddresses = async (user) => {
  ...
  const allAddresses = await addressModel
    .find({ userId })
    .sort({ isDefault: -1 });
  return allAddresses;
};
...
export const editAddressPage = async (addressId) =>
  ...
  return await addressModel.findByIdAndUpdate(
    addressId,
    {
      $set: {
        ...
        phoneNumber: addressData.phoneNumber,
        isDefault:
          addressData.isDefault === true || addressData.isDefault === 'on',
      },
    },
    { new: true }
  );
...
export const deleteAddress = async (userId, addressId) => {
  const isDefault = await addressModel.findOne({
    userId: userId,
    _id: addressId,
    isDefault: true,
  });
...
export const addToCart = async (userId, productId, variantId) => {
  ...
  if (existCart && existCart.quantity >= variant.stock) {
    throw new Error(
      `Stock limit exceed for the ${theProduct.productName}'s this variant,(only ${variant.stock} stock available)`
    );
  }
  if (existCart) {
...
export const itemCancel = async (
  ...
  if (cancellable <= 0) {
    throw new Error(
      'All units of this item have already been cancelled or returned'
    );
  }
  if (qty > cancellable) {
    throw new Error(
...
export const itemReturn = async (
  ...
  if (item.returnStatus === 'requested') {
    throw new Error(
      'A return is already pending for this item. Wait for admin to process it first'
    );
  }
  ...
  if (returnable <= 0) {
    throw new Error(
      'All units of this item have already been returned or cancelled'
    );
  }
  if (qty > returnable) {
    throw new Error(
      `You can only return ${returnable} unit(s). You already returned ${alreadyReturned}`
    );
  }
