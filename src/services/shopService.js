...
export const checkoutPage = async (userId) => {
    try {
        ...
        const checkoutData = await Promise.all(cartData.map(async (item) => {
        ...
        const mrpSubTotal = filteredCheckoutData.reduce((sum, item) => sum + (item.isAvailable ? (item.price * item.quantity) : 0), 0);
        const totalSavings = filteredCheckoutData.reduce((sum, item) => sum + (item.isAvailable ? (item.offerDiscount * item.quantity) : 0), 0);
        
        const couponsAvailable=await couponModel.find({minOrderAmount:{$lte:mrpSubTotal}});
        ...
        console.log(`Checkout page loaded for user ${userId}. Items: ${filteredCheckoutData.length}`);
...
export const placeOrder=async(userId,addressId,paymentMethod,cartItems, couponCode = null)=>{
    ...
    for (let item of cartItems) {
        ...
        const productObj = await productModel.findById(item.productId).populate("category");
        if (!productObj) throw new Error("Product no longer available");

        const variant = productObj.variants.find(v => v._id.toString() === item.variant._id.toString());
        if (!variant || !variant.isListed) throw new Error(`Variant of ${item.productName} no longer available`);
...
export const placeBuyNowOrder=async(userId,addressId,paymentMethod,buyNowItem, couponCode = null)=>{
    const address=await addressModel.findOne({userId:userId,_id:addressId});
    if(!address){
        throw new Error("Invalid shiping address.");
    }
...
export const validateCoupon = async (code, subTotal) => {
    try {
        ...
        if (!coupon) return { isValid: false, message: "Coupon not found or inactive" };
        if (coupon.minOrderAmount > subTotal) return { isValid: false, message: `Minimum order of ₹${coupon.minOrderAmount} required` };
        if (coupon.expiresAt && new Date(coupon.expiresAt) < new Date()) return { isValid: false, message: "Coupon has expired" };
        if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) return { isValid: false, message: "Coupon usage limit reached" };
