...
export const loadProductDetailsePage = asyncHandler(async (req, res) => {
  try {
    ...
    const userId = req?.session?.user?.id || req?.user?.id || '';
    const { product, relatedProducts, wishlist } =
      await shopService.productDetailsePage(productId, userId);
...
export const wishlistUpdate = asyncHandler(async (req, res) => {
  try {
    ...
    console.log(
      `wishlist =product id:${productId}, and variant id:${variantId}, userId:${userId}`
    );
    const update = await shopService.wishlistUpdate(
      productId,
      userId,
      variantId
    );
...
export const addToCart = asyncHandler(async (req, res) => {
  try {
    ...
    const cartCount = await shopService.addToCart(
      userId,
      productId,
      variantId,
      quantity
...
export const proceedToCheckout = asyncHandler(async (req, res) => {
  try {
    ...
    if (isBuyNow && req.session.buyNowItem) {
      ...
      checkout = await shopService.checkoutBuyNowOrder(
        userId,
...
export const placeOrder = asyncHandler(async (req, res) => {
  try {
    const userId = req.session.user.id || req.user.id;
    const { addressId, paymentMethod, productId, variantId, couponCode } =
      req.body;
    ...
    if (paymentMethod !== 'cash') {
      if (productId && variantId) {
        const buyNowData = await shopService.checkoutBuyNowOrder(
          userId,
          req.session.buyNowItem
        );

        let discountedAmount =
          buyNowData.totals.subTotal - (buyNowData.totals.totalSavings || 0);
        if (couponCode) {
          const coupon = await shopService.validateCoupon(
            couponCode,
            discountedAmount
          );
        ...
        const shipping =
          discountedAmount < 1000 && discountedAmount > 0 ? 40 : 0;
        ...
        let discountedAmount =
          ...
          (checkoutData.totals.totalSavings || 0);
        if (couponCode) {
          const coupon = await shopService.validateCoupon(
            couponCode,
            discountedAmount
      ...
      if (paymentMethod === 'razorpay') {
        let order;
        if (productId && variantId) {
          order = await shopService.placeBuyNowOrder(
            userId,
            addressId,
            paymentMethod,
            req.session.buyNowItem
...
const getCartData = async (userId, variantId) => {
  ...
  const cartItems = await cartModel.aggregate([
    ...
    { $unwind: '$product.variants' },
    {
      $match: {
        $expr: {
          $eq: ['$product.variants._id', { $toObjectId: '$variantId' }],
        },
      },
    },
