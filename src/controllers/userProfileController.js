...
export const editPassword = asynchandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: 'password updated successul' });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
});
...
export const wishlistPage = asynchandler(async (req, res) => {
  try {
    ...
    res.render('user/userAfterLogin/wishlist', {
      wishlist,
...
export const removeWishlist = asynchandler(async (req, res) => {
  try {
    ...
    let result = await addressService.deleteWishlistItem(
      userId,
      productId,
      variantId
    );
    res
      .status(200)
      .json({ success: result, message: 'item removed from wishlist' });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
});
...
export const addToCart = asynchandler(async (req, res) => {
  try {
    ...
    const productId = req.params.id;
    const variantId = req.query.variantId;
    let updateToCart = await addressService.addToCart(
      userId,
      productId,
      variantId
    );
...
export const deleteCartItem = asynchandler(async (req, res) => {
  try {
    ...
    const productId = req.params.id;
    const variantId = req.query.variantId;
    const update = await addressService.deleteCart(
      userId,
      productId,
      variantId
    );
...
export const changeCartQty = asynchandler(async (req, res) => {
  try {
    ...
    const userId = req.session?.user.id || req?.user.id;
    const update = await addressService.changeCartQuantity(
      userId,
      change,
      productId,
      variantId,
    ...
    res
      .status(200)
      .json({ success: true, message: 'quantity changed', update: update });
  } catch (error) {
    console.log(error);
    res
      .status(400)
      .json({
        success: false,
        message: error.message || 'cannot change the quatity',
      });
  }
});
...
export const ordercancel = asynchandler(async (req, res) => {
  try {
    ...
    const cancelling = await addressService.orderCancel(
      userId,
      orderId,
      reason,
      comments
    ...
    res
      .status(400)
      .json({ success: false, message: "can't change order status" });
  }
});
...
export const itemCancel = asynchandler(async (req, res) => {
  try {
    ...
    await addressService.itemCancel(
      userId,
      orderId,
      itemId,
      reason,
    ...
    res
      ...
      .json({
        success: false,
        message: error.message || "can't change item status",
      });
  }
});
...
export const itemReturn = asynchandler(async (req, res) => {
  try {
    ...
    await addressService.itemReturn(
      userId,
      orderId,
      itemId,
      reason,
...
export const walletPage = asynchandler(async (req, res) => {
  try {
    ...
    res.render('user/userAfterLogin/wallet', {
      user: req.session.user || req.user,
      wallet,
    });
  } catch (error) {
