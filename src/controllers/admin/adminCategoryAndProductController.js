...
export const getCategory = asynchandler(async (req, res) => {
  try {
    ...
    const limit = 10;
    const { category, totalCountOfCategory } =
      await serviceOfProductAndCategory.categoryData(page, limit, search);
    ...
    res.render('admin/categories', {
      category,
      totalCount: totalCountOfCategory,
      currentPage: page,
      totalPages,
      limit,
      search,
...
export const listAndUnlistCategory = asynchandler(async (req, res) => {
  try {
    ...
    const sevice = await serviceOfProductAndCategory.listAndUnlistCategory(
      categoryId,
      action
    );
...
export const getEditCategoryPage = asynchandler(async (req, res) => {
  try {
    const categoryId = req.params.id;
    const category =
      await serviceOfProductAndCategory.editCategoryPage(categoryId);
...
export const productPage = asynchandler(async (req, res) => {
  try {
    ...
    let limit = 10;
    let { products, totalCountOfProducts } =
      await serviceOfProductAndCategory.productPage(page, limit, search);
...
export const addProduct = asynchandler(async (req, res) => {
  try {
    ...
    let formatedVariants = variantsArray.map((variant, index) => {
      let variantImagesToUpload =
        variantsImageUrls[`variant_${index}_images`] || [];
      return {
    ...
    const uploaded = await serviceOfProductAndCategory.addProduct(
      finalProductDataToUpload
    );
...
export const editProductPage = asynchandler(async (req, res) => {
  try {
    const productId = req.params.id;
    const { productForEdit, category } =
      await serviceOfProductAndCategory.editProductPage(productId);
...
export const updateProduct = asynchandler(async (req, res) => {
  try {
    ...
    console.log(
      `productId: ${productId}, existProduct: ${existProduct}, product name:${productName}`
    );
    if (existProduct) {
    ...
    const update =
      await serviceOfProductAndCategory.updateProduct(finalProductData);
    res
      .status(200)
      .json({ success: true, message: 'product update success..' });
  } catch (error) {
