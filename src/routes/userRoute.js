...
router.get(
  '/ZiGo.com',
  userAuth.isLogout,
  userAuth.checkBlocked,
  userCntrl.LoadHomePage
);
...
router.patch(
  '/user/cart/changeQty',
  userProfile.changeCartQty,
  cartMiddlware.cartCount
);
...
router.patch(
  '/user/order/:orderId/item/:itemId/cancel',
  userProfile.itemCancel
);
router.patch(
  '/user/order/:orderId/item/:itemId/return',
  userProfile.itemReturn
);
